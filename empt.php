...
$Transfer_Money=$_POST['Transfer_Money'];
       ...
       $sql="INSERT INTO empt(Account_Id,Employee_Id,Transfer_Money) VALUES (?,?,?)";
       $stmtinsert=$db->prepare($sql);
    
     $result=$stmtinsert->execute([$Account_Id,$Employee_Id,$Transfer_Money]);
    ...
    // $sql="SELECT  Amount FROM account_detalis SET Amount='Amount+$Transfer_Money' WHERE Account_Id=$Account_Id'";
     ...
     $result=$stmtinsert->execute([$Account_Id,$Employee_Id,$Transfer_Money]);
             ...
             $sql = "UPDATE emp_acc SET Amount = Amount+$Transfer_Money WHERE Employee_Id=$Employee_Id";
            
    
       $stmtinsert=$db->prepare($sql);
    
     $result=$stmtinsert->execute([$Account_Id,$Employee_Id,$Transfer_Money]);
        $sql="UPDATE emp_acc SET Amount = Amount-$Transfer_Money WHERE Account_Id=$Account_Id"; 
           $stmtinsert=$db->prepare($sql);
...
echo '<script>swal("Sucessfully Transfer from your account ")</script>';
...
<html>
    ...
    <body>
        ...
        <form action="empt.php" method="post">
         <div class="input-group">
        
      <label>Enter reciver Employee_Id</label>
